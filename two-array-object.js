...
function twoArrayObject(keysArray, valuesArray) {
    ...
    // first array is keys array
    // second array is values array

    const returnObject = {}

    // if not enough keys - truncate ending values

    if (keysArray.length < valuesArray.length){
        valuesArray = valuesArray.slice(0,(keysArray.length))
    }

    // if not enough values - remaining keys key:null

    for(let i = 0; i < keysArray.length; i++){
        
        if(!valuesArray[i]){
            returnObject[(keysArray[i])] = null;
        } else {
            returnObject[(keysArray[i])] = valuesArray[i]
        }
    ...
    // function should return an object of keys:values
